...
# ==========================================
# 2. 统计核心算法：Bootstrapping & DeLong Test
# ==========================================
def calculate_auc_ci(y_true, y_prob, n_classes, n_bootstrap=1000):
    """使用 Bootstrapping 计算 Macro-average ROC-AUC 的 95% 置信区间"""
    bootstrapped_aucs = []
    y_true_arr = np.array(y_true)
    for i in range(n_bootstrap):
        indices = resample(np.arange(len(y_true_arr)), random_state=42 + i)
        if len(np.unique(y_true_arr[indices])) < n_classes:
            continue
...
plt.title("Macro-average OvR ROC (East Asia) with 95% CI", fontsize=20, fontweight="bold")
plt.xlabel("False Positive Rate", fontsize=18); plt.ylabel("True Positive Rate", fontsize=18)
plt.legend(loc="lower right", fontsize=14); plt.grid(alpha=0.3)
plt.gca().spines['top'].set_visible(False); plt.gca().spines['right'].set_visible(False)
plt.tight_layout()
plt.savefig("Macro_ROC_with_CI_EastAsia.pdf", format='pdf', dpi=600)

# ==========================================
# 5. DeLong Test 显著性矩阵与热力图
# ==========================================
...
ax = sns.heatmap(
    delong_lower,
    annot=delong_lower.map(lambda x: f"{x:.3f}\n{pvalue_star(x)}" if not pd.isna(x) else ""),
    fmt="", cmap="RdYlBu_r", vmin=0, vmax=0.05, square=True, linewidths=1, linecolor="white",
    cbar_kws={"label": "P value (Significant < 0.05)"}
)
ax.set_title("DeLong Test: East Asia ROC Comparison", fontsize=20, fontweight="bold", pad=20)
plt.tight_layout()
plt.savefig("delong_comparison_EastAsia.pdf", format='pdf', dpi=600)
